...
class TuyaCloud {
  ...
  public function getAccessToken() {
    ...
    // ตั้งค่าไฟล์ที่ใช้เก็บ access_token
    $tokenFile = 'access_token.json';

    // ตรวจสอบว่าไฟล์ token มีอยู่หรือไม่
    if (file_exists($tokenFile)) {
        $dataStore = json_decode(file_get_contents($tokenFile));
    ...
    // ถ้าไม่มี token ที่ยังไม่หมดอายุ ต้องขอ token ใหม่
    ...
    if (isset($data['result']['access_token'])) {
      ...
      file_put_contents($tokenFile, json_encode($dataStore));
